adminPage.SelectItinerary = (function() {
	...
	var getRouteCandidates = function(trackingId){
		comm.callApi({
			url: comm.server.url + "/tracker/cargos/candidates",
			method: "GET",
			dataType: "JSON",
...
//action="#{itinerarySelection.assignItinerary(itineraryStatus.index)}"
		...
		doAssignRoute: function(index){
			...
			comm.callApi({
				...
				success: function(data, textStatus, jqXHR){
					console.log(data);
					window.location.href = "#admin/detail/routed/" + _cargoDetail.trackingId;
				},
	...
	return {
		initPage: function(status, id){
		    ...
		    template.RenderOne({
		        target: "#body",
		        tagName: "div",
		        className: "adminSelect-itinerary",
		        id: "bodyAdminSelectItinerary",
		        position: "new",
